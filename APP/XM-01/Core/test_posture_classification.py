# -*- coding: utf-8 -*-
"""
测试睡姿分类模型准确率 - 使用垂直位移增强模型
Test Sleep Posture Classification Model Accuracy - With Vertical Shift Augmentation

使用parsed_sensor_data.csv中的数据测试新的垂直位移增强CNN模型
Use data from parsed_sensor_data.csv to test new CNN model with vertical shift augmentation

新模型特点 (2026-03-03):
- 包含垂直位移数据增强，能够处理人在床上半部分或下半部分的情况
- 使用两个数据源的组合训练数据
- 预期准确率 ≥97%
...
def test_posture_classification():
    ...
    # 确保数据类型正确 - 转换为整数 (2026-03-03)
    results_df['true_posture'] = pd.to_numeric(results_df['true_posture'], errors='coerce').astype('Int64')
    results_df['pred_posture'] = pd.to_numeric(results_df['pred_posture'], errors='coerce').astype('Int64')

    # 调试：检查数据类型和唯一值 (2026-03-03)
    print("\n[调试] DataFrame信息:")
    print(f"  总行数: {len(results_df)}")
    print(f"  true_posture类型: {results_df['true_posture'].dtype}")
    print(f"  pred_posture类型: {results_df['pred_posture'].dtype}")
    print(f"  true_posture唯一值: {sorted(results_df['true_posture'].dropna().unique())}")
    print(f"  pred_posture唯一值: {sorted(results_df['pred_posture'].dropna().unique())}")
    print(f"  true_posture==0的数量: {len(results_df[results_df['true_posture'] == 0])}")
    print(f"  true_posture==1的数量: {len(results_df[results_df['true_posture'] == 1])}")
    print(f"  true_posture有NaN: {results_df['true_posture'].isna().sum()}")
    print(f"  pred_posture有NaN: {results_df['pred_posture'].isna().sum()}")
    ...
    # 调试输出 (2026-03-03)
    print(f"\n[调试] 混淆矩阵原始值:")
    print(f"  TP (真仰卧→预测仰卧): {TP}")
    print(f"  FP (真仰卧→预测侧卧): {FP}")
    print(f"  FN (真侧卧→预测仰卧): {FN}")
    print(f"  TN (真侧卧→预测侧卧): {TN}")
    print(f"  混淆矩阵总和: {TP + FP + FN + TN}")
    ...
    print(f"  [调试] supine_total = TP + FP = {TP} + {FP} = {supine_total}")
    ...
    print(f"  [调试] side_total = FN + TN = {FN} + {TN} = {side_total}")
    ...
    # 错误样本分析 - 从混淆矩阵计算 (2026-03-03)
    error_count = FP + FN
    if error_count > 0:
        print(f"\n错误样本数: {error_count}")

        # 找出错误样本 (2026-03-03)
        error_samples = results_df[
            ((results_df['true_posture'] == 0) & (results_df['pred_posture'] == 1)) |
            ((results_df['true_posture'] == 1) & (results_df['pred_posture'] == 0))
        ]

        print("\n前10个错误样本:")
        print(error_samples.head(10)[['filename', 'true_posture', 'pred_posture', 'probability', 'side']])
